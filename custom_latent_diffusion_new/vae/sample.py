...
def sample():
    parser = argparse.ArgumentParser(description="Sample from variational autoencoder")
    parser.add_argument(
        "--run-name",
        required=True,
    ...
    _, _, _, _, _, test_loader, _, _ = build_splits_and_loaders(
    ...
    image = image[random_idx : random_idx + 1].to(device)
